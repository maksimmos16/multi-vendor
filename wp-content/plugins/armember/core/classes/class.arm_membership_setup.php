        ...
        function arm_setup_shortcode_func_internal($atts, $content = "") {
            ...
            if ((!empty($args['id']) && $args['id'] != 0) || ($isPreview && !empty($args['setup_data']))) {
                ...
                if (!empty($setup_data) && !empty($setup_data['setup_modules']['modules'])) {
                    ...
                    $formPosition = (!empty($setup_style['form_position'])) ? $setup_style['form_position'] : 'left';
                            ...
                            case 'forms':
                                ...
                                if (!empty($modules['forms']) && $modules['forms'] != 0) {
                                    if (!empty($form_settings)) {
                                        ...
                                        $errPosCCField = (!empty($form_settings['style']['validation_position']) && $form_settings['style']['validation_type'] != 'standard') ? $form_settings['style']['validation_position'] : 'bottom';
                    ...
                    $content .= '<form method="post" name="arm_form" id="arm_setup_form' . $setupRandomID . '" class="arm_setup_form_' . $setupID . ' arm_membership_setup_form arm_form_' . $modules['forms'] . $form_layout . ' ' . $is_form_class_rtl . '" enctype="multipart/form-data" data-random-id="' . $setupRandomID . '" novalidate ' . $form_attr . '>';
            ...
            if(is_user_logged_in()) {
                ...
                if ($form->exists() && !empty($form->fields)) 
                {
                    ...
                    if($arm_user_payment_gateway=='stripe' || $arm_user_payment_gateway=='authorize_net' || $arm_allow_gateway)
                    {
                        ...
                        $arm_update_card_form .= $arm_payment_gateways->arm_get_credit_card_box($arm_user_payment_gateway, $column_type, $fieldPosition, $errPosCCField, $form_style['form_layout']);
